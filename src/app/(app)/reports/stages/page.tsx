// src/app/(app)/reports/stages/page.tsx
...
async function fetchStages(params: {
  ...
  if (params.place && params.place !== "all") {
    q1 = q1.eq("location", params.place);
  }

  // szukaj po nazwie zadania / miejscu / ID raportu
  if (params.q && params.q.trim()) {
    ...
    const { data, error } = await supabase
      ...
      .select("*")
      .or(
        `task_name.ilike.%${qText}%,location.ilike.%${qText}%,report_id.ilike.%${qText}%`
      )
      .order("location", { ascending: true })
      .order("task_name", { ascending: true });

    if (error) {
      console.error("fetchStages search error:", error);
      return [];
    }
